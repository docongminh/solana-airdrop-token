...
export class SPLToken  {
  ...
  async transfer(
    ...
    wallet: Wallet
      ...
      const rawAmount = parseUnits(transferParams.amount, this._decimal).toNumber()
      //
      ...
      const info = await this._connection.getParsedAccountInfo(associatedAccountReceiver);
      const associatedAcccountInstruction: TransactionInstruction[] = [];
      if(info.value == null){
        // create account instructions
        associatedAcccountInstruction.push(Token.createAssociatedTokenAccountInstruction(
          ASSOCIATED_TOKEN_PROGRAM_ID,
          TOKEN_PROGRAM_ID,
          new PublicKey(this._mintAddress),
      ...
      const transferTokenRawInstruction =
